...
void Context::ScaleAnimationToWindow(std::shared_ptr<Util::Animation> animation) {
    if (animation == nullptr) {
        LOG_ERROR("Animation is null. Cannot scale.");
        return;
    }
    ...
    for (const auto &frame : animation->GetFrames()) {
        if (frame == nullptr) {
            LOG_ERROR("Frame is null. Skipping.");
            continue;
        }
        frame->SetSize(glm::vec2(windowWidth, windowHeight));
    }

    LOG_INFO("Animation scaled to window size: {}x{}", windowWidth, windowHeight);
}
}
// namespace Core
